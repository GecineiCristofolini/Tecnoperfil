...
public class BotaoImprimirDeclaracaoProdutor extends StringConverter {
	...
	private String getBotao(EFormField field, OriginEnum origin) {
		...
		String botao = "<script>" 
				...
				+ "var url = '"
				+ url
				+ "servlet/ExportacaoServeletUtilsDocumentoProdutor?action=geraRelatorioprodutor&pedido="
				           
				+ field.getForm().getObject().getNeoId()
				+ "';"
				+ "window,open(url);"
				+ "} </script> "
				+ "<input type='button' class='input_button' value='Exportar Declaracao Produtor' onclick='opnreportDocumento()'>";
		return botao;
	}
