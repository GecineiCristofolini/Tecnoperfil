...
public class ArvoreTabelaConverterTotvs extends StringConverter {
		...
		if (itens != null && !itens.isEmpty()) {
			...
			for (NeoBaseEntity item : itens) {
				...
				if(valorUniDesc.floatValue() > valorUni.floatValue())   {
					...
					appendShortTag("td", HidroponiaSnapshotpedidoTotvs.formatCurrencyValue(itemWrapper.findGenericValue("PreComDes")), "preuni", sb);
				
				    }
				 
				else{
					appendShortTag("td", HidroponiaSnapshotpedidoTotvs.formatCurrencyValue(itemWrapper.findGenericValue("PreUniH")), "preuni", sb);
				}
